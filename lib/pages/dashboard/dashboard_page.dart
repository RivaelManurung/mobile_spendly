...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Widget _buildContent(AppState state) {
    switch (_activeTabIndex) {
      case 0: return _GroupedTransactionList(state: state);
      case 1: return const SliverFillRemaining(child: Center(child: Icon(Icons.calendar_month, size: 64, color: AppTheme.textMuted)));
      case 2: return const SliverFillRemaining(child: Center(child: Text('Tampilan Bulanan')));
      case 3: return const SliverFillRemaining(child: Center(child: Text('Ringkasan Laporan')));
      case 4: return const SliverFillRemaining(child: Center(child: Text('Deskripsi Keuangan')));
      default: return _GroupedTransactionList(state: state);
    }
  }
}
              ...
              child: Column(
                ...
                children: [
                  Text(
                    ...
                    style: AppTheme.geist(size: 9, w: FontWeight.w500, color: AppTheme.textMuted, spacing: 1.2),
                  ),
                  ...
                  FittedBox(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(
                          state.userName,
                          style: AppTheme.geist(size: 26, w: FontWeight.w500, spacing: -0.3),
                        ),
...
class _DateSelectorRow extends StatelessWidget {
  ...
  const _DateSelectorRow({required this.activeIndex, required this.onTabChanged});
          ...
          children: [
            Row(
              ...
              children: [
                ...
                Text(
                  Fmt.currentMonthYear().toUpperCase(),
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: [
                  _TabItem(label: 'Harian', isActive: activeIndex == 0, onTap: () => onTabChanged(0)),
                  _TabItem(label: 'Kalender', isActive: activeIndex == 1, onTap: () => onTabChanged(1)),
                  _TabItem(label: 'Bulanan', isActive: activeIndex == 2, onTap: () => onTabChanged(2)),
                  _TabItem(label: 'Ringkasan', isActive: activeIndex == 3, onTap: () => onTabChanged(3)),
                  _TabItem(label: 'Deskripsi', isActive: activeIndex == 4, onTap: () => onTabChanged(4)),
                ],
              ),
            ),
    ...
    return SliverToBoxAdapter(
      child: Container(
        ...
        child: Row(
          ...
          children: [
            _SumColumn(label: 'Pendapatan', value: stats['income'] ?? 0, color: Colors.blueAccent),
            _SumColumn(label: 'Pengeluaran', value: stats['expense'] ?? 0, color: AppTheme.rose),
            _SumColumn(label: 'Total', value: stats['net'] ?? 0, color: AppTheme.textPrimary, isBold: true),
          ],
        ),
      ),
    );
...
class _SumColumn extends StatelessWidget {
  ...
  final Color color;
  final bool isBold;
  const _SumColumn({required this.label, required this.value, required this.color, this.isBold = false});
  ...
  Widget build(BuildContext context) {
    if (state.transactions.isEmpty) {
      return SliverFillRemaining(
        hasScrollBody: false,
        child: Center(
          child: Text('Belum ada transaksi', style: AppTheme.geist(color: AppTheme.textMuted)),
        ),
      );
    }
    ...
    for (var tx in state.transactions) {
      final dateKey = '${tx.date.day.toString().padLeft(2, '0')} ${Fmt.weekday(tx.date)}';
    ...
    return SliverList(
      delegate: SliverChildBuilderDelegate(
        (context, index) {
          ...
          final dayIncome = txs.where((t) => t.type == TransactionType.income).fold(0.0, (s, t) => s + (t.amount));
          final dayExpense = txs.where((t) => t.type == TransactionType.expense).fold(0.0, (s, t) => s + (t.amount));
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: Text(parts[1], style: AppTheme.geist(size: 10, color: AppTheme.textPrimary, w: FontWeight.w700)),
          ),
          const Spacer(),
          Text(
            Fmt.idr(netBalance), 
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Text('Other', style: AppTheme.geist(size: 13, color: AppTheme.textMuted)),
            const SizedBox(width: 40),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(tx.title, style: AppTheme.geist(size: 15, w: FontWeight.w500)),
                  Text('Cash', style: AppTheme.geist(size: 11, color: AppTheme.textMuted)),
                ],
              ),
            ),
            Text(
              ...
              style: AppTheme.mono(
                size: 13, 
                color: tx.type == TransactionType.income ? Colors.blueAccent : AppTheme.rose
              ),
            ),
          ],
        ),
